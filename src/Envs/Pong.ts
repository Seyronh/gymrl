...
class Pong extends Env {
  private player: number;
  private difficulty: number;
  private player1: [number, number] = [0, 5];
  private player2: [number, number] = [10, 5];
  private ball: [number, number] = [5, 5];
  private balldir: [number, number] = [1, 1];
  /**
  ...
  step(action: number): [Observation, Reward, Done, Info] {
    ...
    if (this.player == 1) {
      const right = Math.random() > 1 - this.difficulty;
      if (right) {
  ...
  processball() {
    ...
    if (
      (this.ball[0] - 1 == this.player1[0] &&
        (this.ball[1] == this.player1[1] ||
          this.ball[1] == this.player1[1] - 1 ||
          this.ball[1] == this.player1[1] + 1)) ||
      (this.ball[0] + 1 == this.player2[0] &&
        (this.ball[1] == this.player2[1] ||
          this.ball[1] == this.player2[1] - 1 ||
          this.ball[1] == this.player2[1] + 1))
    ) {
      this.balldir[0] *= -1;
    }
    this.ball = [
      this.ball[0] + this.balldir[0],
      this.ball[1] + this.balldir[1],
    ];
  }
  /**
